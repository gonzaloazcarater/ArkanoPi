...
void InicializaTeclado(TipoTeclado *p_teclado) {
	...
	//Excitacion por columnas
	pinMode (GPIO_KEYBOARD_COL_1, OUTPUT); //Se declara puerto de salida
	digitalWrite (GPIO_KEYBOARD_COL_1, HIGH); //Se excita la columna poniendo un 1 (HIGH)
	...
	pinMode (GPIO_KEYBOARD_ROW_1, INPUT); //Se declara puerto de entrada
	pullUpDnControl(GPIO_KEYBOARD_ROW_1, PUD_DOWN); //Protegemos las entradas con resistencias de pull up and down para no dejar las entradas al aire pq actuarían como una antena y podria detectar pulsaciones no deseadas
	wiringPiISR (GPIO_KEYBOARD_ROW_1, INT_EDGE_RISING, teclado_fila_1_isr); //interrupcion en flanco de subida que se atiende con la isr del parametro 3
	...
	printf("\nEl teclado se ha inicializado correctamente.\n\n");
	fflush(stdout);
...
void ProcesaTeclaPulsada (fsm_t* this) {
	...
	switch(p_teclado->teclaPulsada.col){
		...
		case COLUMNA_1:
			if(p_teclado->teclaPulsada.row == FILA_4){ //Tecla d
		...
		case COLUMNA_3:
			...
			if(p_teclado->teclaPulsada.row == FILA_1){ //Tecla 3

				piLock(SYSTEM_FLAGS_KEY);
				flags |= FLAG_PAUSA; //Activamos flag para que se atienda la interrupción
				piUnlock(SYSTEM_FLAGS_KEY);
			}
